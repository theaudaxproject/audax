...
VERSION=
commit=false
url=https://github.com/audaxproject/audax
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the audax, gitian-builder, gitian.sigs, and audax-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/audaxproject/audax
...
then
	# Make output folder
	mkdir -p "./audax-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit audax=${COMMIT} --url audax=${url} ../audax/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../audax/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/audax-*.tar.gz build/out/src/audax-*.tar.gz ../audax-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit audax=${COMMIT} --url audax=${url} ../audax/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/audax-*.zip build/out/audax-*.exe ../audax-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit audax=${COMMIT} --url audax=${url} ../audax/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/audax-*.tar.gz build/out/audax-*.dmg ../audax-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit audax=${COMMIT} --url audax=${url} ../audax/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../audax/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/audax-*.tar.gz build/out/src/audax-*.tar.gz ../audax-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../audax/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/audax-*win64-setup.exe ../audax-binaries/${VERSION}
	    mv build/out/audax-*win32-setup.exe ../audax-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../audax/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/audax-osx-signed.dmg ../audax-binaries/${VERSION}/audax-${VERSION}-osx.dmg
	fi
	popd || exit
