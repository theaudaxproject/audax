...
#if defined(NDEBUG)
#error "Audax cannot be compiled without assertions."
#endif
...
{
    RenameThread("audax-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateAUDAXSupply(int nHeightStart)
{
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (!fLiteMode) {
        if (masternodeSync.RequestedMasternodeAssets > MASTERNODE_SYNC_LIST || Params().NetworkID() == CBaseChainParams::REGTEST) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/Audax Core:1.1.0/" ||
            pfrom->cleanSubVer == "/Audax Core:1.3.0/" ||
            pfrom->cleanSubVer == "/Audax Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // Audax: We use certain sporks during IBD, so check to see if they are
