...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::AUDAX);
    ...
    if (!settings.contains("nAnonymizeAudaxAmount"))
        settings.setValue("nAnonymizeAudaxAmount", 1000);

    nAnonymizeAudaxAmount = settings.value("nAnonymizeAudaxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAudaxAmount"))
        SoftSetArg("-anonymizeaudaxamount", settings.value("nAnonymizeAudaxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in audax.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAudaxAmount:
            return QVariant(nAnonymizeAudaxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAudaxAmount:
            nAnonymizeAudaxAmount = value.toInt();
            settings.setValue("nAnonymizeAudaxAmount", nAnonymizeAudaxAmount);
            emit anonymizeAudaxAmountChanged(nAnonymizeAudaxAmount);
            break;
        case CoinControlFeatures:
