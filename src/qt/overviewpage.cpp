...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::AUDAX)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, QString& sAUDAXPercentage)
{
    int nPrecision = 2;
    ...
    sAUDAXPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelAUDAXPercent->setText(sPercentage);
    ...
    bool showAUDAXAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyAUDAXAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showAUDAXPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyAUDAXPending = watchUnconfBalance != 0;
    bool showAUDAXLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyAUDAXLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showAUDAXAvailable || showWatchOnlyAUDAXAvailable);
    ui->labelBalanceText->setVisible(showAUDAXAvailable || showWatchOnlyAUDAXAvailable);
    ui->labelWatchAvailable->setVisible(showAUDAXAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showAUDAXPending || showWatchOnlyAUDAXPending);
    ui->labelPendingText->setVisible(showAUDAXPending || showWatchOnlyAUDAXPending);
    ui->labelWatchPending->setVisible(showAUDAXPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showAUDAXLocked || showWatchOnlyAUDAXLocked);
    ui->labelLockedBalanceText->setVisible(showAUDAXLocked || showWatchOnlyAUDAXLocked);
    ui->labelWatchLocked->setVisible(showAUDAXLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("AUDAX")
    updateDisplayUnit();
}
