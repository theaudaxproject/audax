...
class Node :
	...
	def createDataDir(self, nodeCount, masterNodePrivateKey = None) :
		...
		writeFile(
			os.path.join(self._nodePath, 'audax.conf'),
			self._generateAudaxConf(nodeCount, masterNodePrivateKey)
		)
		
	def startNode(self) :
		self._daemonProcess = subprocess.Popen([ self._audaxd, '-datadir=' + self._nodePath, '-daemon' ])
	...
	def executeCli(self, *args) :
		...
		output = executeCommand(self._audaxCli, '-datadir=' + self._nodePath, *normalizedArgs)
	...
	def _generateAudaxConf(self, nodeCount, masterNodePrivateKey) :
		result = ""
		result += "regtest=1\n"
		result += "server=1\n"
		result += "debug=1\n"
